...
async function createUserClient(email, password) {
  ...
  if (error && error.message.includes('already registered')) {
    ...
    return {
      ...
      client: createClient(SUPABASE_URL, process.env.VITE_SUPABASE_ANON_KEY, {
...
async function runVerification() {
  ...
  try {
    ...
    const { data: adminSignIn } = await adminClient.auth.signInWithPassword({
      email: adminEmail,
      password,
    });
    ...
    const authenticatedAdminClient = createClient(
      SUPABASE_URL,
    ...
    const { data: coachSignIn } = await adminClient.auth.signInWithPassword({
      email: coachEmail,
      password,
    });
    const authenticatedCoachClient = createClient(
      SUPABASE_URL,
      process.env.VITE_SUPABASE_ANON_KEY,
      {
    ...
    if (!coachSeasonError)
      throw new Error('❌ SECURITY FAILURE: Coach was able to create a season!');
    ...
    if (coachAssignmentsBefore.length > 0)
      throw new Error('❌ SECURITY FAILURE: Coach saw practice for team they do not coach!');
    ...
    if (coachAssignmentsAfter.length === 0)
      throw new Error('❌ LOGIC FAILURE: Assigned coach CANNOT see their team practices!');
