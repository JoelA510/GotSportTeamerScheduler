...
| Function | Declared in | Purpose |
| --- | --- | --- |
| `public.submit_registration(p_organization_id uuid, p_form_id uuid, p_profile_id uuid, p_responses jsonb, p_player_id uuid default null, p_first_name text default null, p_last_name text default null) returns uuid` | `20260421051109_add_registration_forms_division.sql` (supersedes `20260310000003` and `20260324000003`) | Derives the authoritative `organization_id` and `division_id` from the form (does *not* trust client input), enforces `is_org_member`, allows admins to submit on behalf of others, creates a `players` row if needed, links `profile_players`, and writes the `registrations` row. Emits a `registration.submitted` audit event. |
...
4. **`is_org_member()` / `is_org_admin()` gate.** Called *before* any mutation. Exceptions must carry `ERRCODE = '42501'` so the Supabase client surfaces a structured `403` rather than a generic error.
...
- **`check_password_length_on_auth_users` is not search_path-pinned.** It is `SECURITY DEFINER` and was pinned by `20260421001209`, but it targets `auth.users` rather than a public table; low-risk since the trigger body does no schema-sensitive lookups, but worth noting for a future audit.
