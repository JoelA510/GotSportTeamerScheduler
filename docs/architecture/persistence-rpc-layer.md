...
| `public.admin_create_location(...)`, `public.admin_create_field(...)`, `public.admin_update_field(...)`, `public.admin_delete_field(...)` | `20260504060000_admin_facility_mutation_rpcs.sql` | Admin-only `/fields` facility mutations. Keep `locations`, `fields`, `field_subunits`, and `practice_slots` readable to org members while routing location/field writes through org-scoped RPCs with location ownership checks and `settings.updated` audit events. |
