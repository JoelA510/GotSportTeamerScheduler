...
| `public.persist_practice_schedule(run_data jsonb, assignments jsonb) returns uuid`            | `20260503010000_repair_practice_persistence_rpc.sql` | Upserts the org-scoped `scheduler_runs` row and bulk-upserts validated `practice_assignments` (keyed by `team_id`, `practice_slot_id`, `effective_date_range`). Returns the scheduler run id. |
