...
- **Authentication**: Same JWT + role allow-list + Zod payload as the other persistence functions; resolves `game_slot_id → game_slots → organization_id` and asserts each is in the caller's org set.
- **Rate limit**: `checkRateLimit(user.id)` — default 60 req/min.
- **RPC used**: None today — games are written against `games` with the service-role client inside the function body. Flagged in [`persistence-rpc-layer.md`](./persistence-rpc-layer.md) §5 as an asymmetry to close in v1.1.
...
- **`game-persistence` has no RPC wrapper.** Writes bypass the RPC layer and hit `games` directly under the service-role client. See [`persistence-rpc-layer.md`](./persistence-rpc-layer.md) §5 for the asymmetry.
