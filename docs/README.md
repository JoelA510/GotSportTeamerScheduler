...
- [Waivers](WAIVERS.md) — Exceptions to constraints as records with a lifecycle: scope composition, the `waived`/`clean` disposition, dormancy detection by counterfactual, row annotations for published output, and the narrow coach-travel evaluator that makes incident 9 testable.
