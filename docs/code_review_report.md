...
- **[HIGH] Timezone UTC/Local Conversion Bug:** In `practiceScheduling.js`, time filtering utilizes `new Date(start.toLocaleString('en-US', { timeZone: timezone }))`. This parses the string relative to the _local system runtime_ rather than mapping to UTC properly, which will induce timezone bleed against `season_settings.timezone`.
...
- **[HIGH] Missing React Hook Vitest Coverage:** Out of 12 distinct React hooks in `frontend/src/hooks` (e.g., `useTeamPortal`, `useFields`, `usePracticeAssignments`), _only one_ (`usePermission`) has a correlating Vitest unit test. The rest are entirely untested.
