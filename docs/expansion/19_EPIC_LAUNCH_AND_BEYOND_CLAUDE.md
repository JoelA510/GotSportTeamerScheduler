...
_Goal: Every PR is automatically validated. The `main` branch is always deployable._
...
_Goal: Unskip the final E2E test and build the drag-and-drop Game Scheduling grid._
...
_Goal: Graduate from `mockSupabase.js` to a real Supabase staging environment while keeping all tests green._
