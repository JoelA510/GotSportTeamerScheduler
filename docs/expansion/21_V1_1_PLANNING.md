...
| `frontend/src/contexts/ImportContext.jsx` — `startImport` | Writes an `import_jobs` row, invokes validation, calls apply RPCs for player, coach, and field imports, can defer coach/field apply into `ready_to_apply`, materializes reciprocal player-import buddy pairs, then captures player-import coach volunteer leads. Team import promotion remains pending. |
