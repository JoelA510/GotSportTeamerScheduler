...
| `frontend/src/contexts/ImportContext.jsx` — `startImport` | Writes an `import_jobs` row, invokes validation, calls apply RPCs for player, coach, and field imports, then captures player-import coach volunteer leads. Team and buddy-pair promotion remain pending.             |
