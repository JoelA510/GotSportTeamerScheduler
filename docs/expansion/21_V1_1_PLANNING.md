...
| `frontend/src/contexts/ImportContext.jsx` — `startImport` | Writes an `import_jobs` row, invokes validation, calls apply RPCs for player, coach, and field imports, materializes reciprocal player-import buddy pairs, then captures player-import coach volunteer leads. Team import promotion remains pending. |
