-- Fix: finalize_field_availability_import_job inserted an explicit NULL into
-- import_application_records.applied_payload for the profile_formats,
-- blackout_windows, equipment_requirements and scenario_members ledger rows.
-- That column is `jsonb NOT NULL DEFAULT '{}'::jsonb`, and an explicit NULL
-- bypasses the column default, so finalizing any field-availability job that
-- produced child rows aborted with a not-null violation -- the finalize
-- feature could never complete. Pass '{}'::jsonb (the column's own default)
-- for those ledger rows. Behaviour is otherwise identical: the profile ledger
-- row still carries the full snapshot, and rollback only keys off the
-- field_availability_profiles ledger rows.
BEGIN;
