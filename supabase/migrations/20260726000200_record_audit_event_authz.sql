...
-- _shared/auth.ts's recordAudit() fire-and-forget helper calls this RPC via
-- a service-role client from Edge Functions, where auth.uid() is NULL (no
-- end-user JWT context) -- is_org_member() would always be false for that
-- caller and this migration would otherwise break every Edge-Function-
-- initiated audit write. This mirrors the established
-- v_effective_role <> 'service_role' exemption pattern already used in
...
-- reachable with the secret service-role key (never shipped to the
-- browser), so trusting it here does not reopen the client-exploitable gap
