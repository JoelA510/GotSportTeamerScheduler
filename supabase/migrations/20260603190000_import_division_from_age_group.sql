-- Import: derive a player's division from the GotSport `age group` column.
--
-- GotSport player exports carry the player's age (e.g. 7) but no division /
-- group / program column, so finalize_import_job left every imported player
-- with division_id = NULL and teaming had nothing to group. This redefines
-- finalize_import_job to fall back to `age group` (age N -> division "U(N+1)")
-- when no explicit division column is present. Existing divisions for the
-- season are matched by name as before; only the division_name derivation
-- changed. (Auto-creating missing divisions per season is a follow-up.)
    ...
    WITH pending AS MATERIALIZED (
        SELECT
            ...
            COALESCE(
                ...
                -- GotSport exports the player's age (e.g. 7) with no division/group
                -- column; a player of age N plays in division U(N+1).
                CASE
