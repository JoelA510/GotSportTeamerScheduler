...
export function validateTeam(team, index) {
  if (!team || typeof team !== 'object') {
    throw new TypeError(
      `each team must be an object${index !== undefined ? ` at index ${index}` : ''}`
    );
  }
  if (!team.id) {
    throw new TypeError(
      `each team requires an id${index !== undefined ? ` at index ${index}` : ''}`
    );
  }
  if (!team.division) {
...
export function validateSlot(slot, index) {
  if (!slot || typeof slot !== 'object') {
    throw new TypeError(
      `each slot must be an object${index !== undefined ? ` at index ${index}` : ''}`
    );
  }
  if (!slot.id) {
    throw new TypeError(
      `each slot requires an id${index !== undefined ? ` at index ${index}` : ''}`
    );
  }
...
export function validateAssignment(assignment, index) {
  if (!assignment || typeof assignment !== 'object') {
    throw new TypeError(
      `assignments must contain objects${index !== undefined ? ` at index ${index}` : ''}`
    );
  }
  if (typeof assignment.weekIndex !== 'number' || assignment.weekIndex <= 0) {
    throw new TypeError(
      `assignment.weekIndex must be a positive number${index !== undefined ? ` at index ${index}` : ''}`
    );
  }
