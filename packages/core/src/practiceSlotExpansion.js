...
export function expandPracticeSlotsForSeason({ slots, seasonPhases }) {
  ...
  for (const slot of slots) {
    ...
    const slotValidFrom = slot.validFrom
      ? parseDateOnly(slot.validFrom, `slot ${slot.id} validFrom`)
      : seasonStart;
    const slotValidUntil = slot.validUntil
      ? parseDateOnly(slot.validUntil, `slot ${slot.id} validUntil`)
      : seasonEnd;
    ...
    for (const phase of normalizedPhases) {
      const effectiveRange = intersectDateRanges(
        slotValidFrom,
        slotValidUntil,
        phase.startDate,
        phase.endDate
      ...
      const resolvedCapacity =
        override?.capacity !== undefined
          ? normalizeCapacity(override.capacity, slot.id, phase.id)
          : capacity;
...
function resolveTimeRange({ baseTimeRange, override, slotId, phaseId }) {
  ...
  if (override) {
    if (override.startTime !== undefined) {
      startMinutes = parseTimeOfDay(
        override.startTime,
        `override for slot ${slotId} phase ${phaseId} startTime`
      );
    }
    if (override.endTime !== undefined) {
      endMinutes = parseTimeOfDay(
        override.endTime,
        `override for slot ${slotId} phase ${phaseId} endTime`
      );
    } else if (override.durationMinutes !== undefined) {
      const duration = Number(override.durationMinutes);
      if (!Number.isFinite(duration) || duration <= 0) {
        throw new TypeError(
          `override for slot ${slotId} phase ${phaseId} durationMinutes must be positive`
        );
      }
