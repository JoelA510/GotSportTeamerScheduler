...
export function summarizeTeamGeneration(result) {
  ...
  for (const divisionId of divisionIds) {
    ...
    const { playersAssigned, totalCapacity, slotsRemaining } = rosterBalance.teamStats.reduce(
      (stats, entry) => {
...
function buildOverflowRollup({ entries, summary }) {
  ...
  const players = Number.isFinite(summary.totalPlayers) ? summary.totalPlayers : derived.players;
  const units = Number.isFinite(summary.totalUnits) ? summary.totalUnits : derived.units;
