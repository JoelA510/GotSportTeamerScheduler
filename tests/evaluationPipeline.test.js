...
test('aggregates practice and game evaluations with issue rollups', () => {
  ...
  assert.equal(messages.filter((message) => message.includes('alert threshold')).length, 1);
  ...
  assert.deepEqual(manualFollowUpIssue.details.unassignedByReason, [
    {
      ...
      count: 1,
      teamIds: ['team-3'],
      divisionBreakdown: [{ division: 'U10', count: 1, percentage: 1 }],
    },
  ]);
  ...
  assert.equal(categorySummaryIssue.message, 'Manual follow-up categories: capacity (1 – 100%)');
...
test('game conflict warnings escalate to error severity', () => {
  ...
  const conflictIssue = result.issues.find(
    (issue) => issue.category === 'games' && issue.message.includes('overlapping games')
  );
...
test('unscheduled matchups are surfaced as errors', () => {
  ...
  const unscheduledIssue = result.issues.find(
    (issue) => issue.category === 'games' && issue.message.includes('could not be scheduled')
  );
...
test('shared slot imbalance warnings surface with warning severity', () => {
      ...
      sharedSlotUsage: [
        {
          ...
          fieldId: 'field-x',
          divisionUsage: [{ division: 'U10', count: 3 }],
        },
        {
  ...
  const imbalanceIssue = result.issues.find(
    (issue) => issue.category === 'games' && issue.message.includes('imbalanced across divisions')
  );
...
test('underutilized practice base slots surface as warnings', () => {
  ...
  const warning = result.issues.find((issue) => issue.message.includes('underutilized'));
  assert.ok(warning);
...
test('practice day concentration triggers a warning for limited coverage', () => {
  ...
  const dayWarning = result.issues.find(
    (issue) => issue.category === 'practice' && issue.message.includes('concentrated on Mon')
  );
