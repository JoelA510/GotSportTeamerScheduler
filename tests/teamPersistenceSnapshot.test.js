...
test('prepareTeamPersistenceSnapshot returns payload counts and enriched metadata', () => {
  ...
  const manualAssignments = [{ teamId: 'u8-2', playerId: 'p4', source: 'manual', role: 'captain' }];
  ...
  assert.strictEqual(
    snapshot.pendingManualOverrideGoal,
    'Clear pending coach approvals before syncing.'
  );
...
test('normalizeManualOverrides validates structure and status', () => {
  ...
  assert.throws(
    () => normalizeManualOverrides([{ teamId: 't3', status: 'unknown' }]),
    /unsupported status/
  );
});
...
test('deriveAppliedTeamOverrides normalizes status casing and whitespace', () => {
  const overrides = [{ teamId: 't1', field: 'name', value: 'Tigers', status: '  APPLIED ' }];
...
test('deriveAppliedTeamOverrides treats missing status as pending', () => {
  const overrides = [{ teamId: 't1', field: 'name', value: 'Tigers' }];
