...
test('buildTeamRows normalizes generator output and applies overrides', () => {
  ...
  const {
    ...
    teamOverrides: [{ teamId: 'u10-2', name: 'Renamed', coachId: '  coach-override  ' }],
    runId: 'run-123',
  });
...
test('buildTeamPlayerRows merges manual assignments and enforces validation', () => {
  ...
  assert.throws(
    () => buildTeamPlayerRows({ teamsByDivision: null }),
    /teamsByDivision must be an object/
  );
  assert.throws(
    () =>
      buildTeamPlayerRows({
...
test('persistTeamPlayers inserts rows via Supabase client', async () => {
  ...
  const teamsByDivision = {
    U9: [{ id: 'u9-1', name: 'U9 Team 1', coachId: 'coach-1', players: [{ id: 'p1' }] }],
  };
  ...
  const teamPlayerRowsCall = calls.find((call) => call.table === 'team_players' && call.rows);
