...
const initialMockData = {
  ...
  // CRITICAL FIX: Added organization_members so useOrganization hook works
...
// Expose to window for Playwright E2E manipulation
if (typeof window !== 'undefined') {
  window.__MOCK_DB__ = JSON.parse(JSON.stringify(initialMockData));
}

const getMockData = (table, col, val) => {
  const db = typeof window !== 'undefined' && window.__MOCK_DB__ ? window.__MOCK_DB__ : initialMockData;
